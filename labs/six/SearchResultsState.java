...
public class SearchResultsState implements State {
	...
	public SearchResultsState(AuctionService auction, Scanner buf, String user, ClientGui gui) {
		this.gui = gui;
		this.buf = buf;
	...
	public void show() {
		...
		System.out.println(String.format("%-18s%-100s%-50s%-12s%-12s%-15s%s\n", "Id", "Name", "Description", "CurrentBid", "Owner", "BidsLeft", "EndsBy"));
	...
	public State next() {
		...
		switch(input) {
			...
			case"3": {
				...
				try {
					Auction a = auction.retreive(i);
					if(a.getCreator().equals(user)) {
						if(a.getHasBid() == false) {
							...
							auc.setCurrentBid(Double.parseDouble(price));
			...
			case"4":
				...
				Long i = Long.parseLong(id);
				try {
					//if(auction.retreive(i).getCreator().equals(user) || auction.retreive(i).getHasBid() == false) {
						auction.delete(i);
//					}
//					else {
						//System.out.println("Could not delete this auction.");
					//}
				}
				catch(ObjectNotFoundException e) {
